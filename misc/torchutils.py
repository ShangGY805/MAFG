...
def show_feature_map(feature_map,epoch):  # feature_map=torch.Size([1, 64, 55, 55]),feature_map[0].shape=torch.Size([64, 55, 55])
    ...
    for index in range(1, feature_map_num + 1):  # 通过遍历的方式，将64个通道的tensor拿出
        ...
        plt.imsave('attentionmap/Conv_A/'+str(index) + "_" +str(epoch) + ".png", feature_map[index - 1].cpu().numpy(),cmap='jet')

    # plt.show()

# def single_attention_map(feature_map,epoch):  # feature_map=torch.Size([1, 64, 55, 55]),feature_map[0].shape=torch.Size([64, 55, 55])
#
#
#     # feature_map[2].shape     out of bounds
#     feature_map = feature_map.squeeze(0)  # 压缩成torch.Size([64, 55, 55])
...
#     plt.figure()
#     index = 59
#     plt.subplot(int(row_num), int(row_num), 1)
#     # plt.imshow(feature_map[index - 1], cmap='gray')  # feature_map[0].shape=torch.Size([55, 55])
#     # 将上行代码替换成，可显示彩色
#     plt.imshow(transforms.ToPILImage()(feature_map[index - 1].cpu().numpy()),cmap='jet')#feature_map[0].shape=torch.Size([55, 55])
#     plt.axis('off')
#     # scipy.misc.imsave('feature_map_save//' + str(index) + ".png", feature_map[index - 1])
#     plt.imsave('attentionmap/1/'+str(index) + "_" +str(epoch) + ".png", feature_map[index - 1].cpu().numpy(),cmap='jet')
#
#     # plt.show()
...
#
#     # feature_map[2].shape     out of bounds
#     feature_map = feature_map.squeeze(0)  # 压缩成torch.Size([64, 256, 256])
#     feature_map = torch.mean(feature_map, dim=0)   #压缩成[1,256,256],通道数变为1
#     # feature_map = feature_map.view(1, feature_map.shape[0], feature_map.shape[1])  # (1,64,55,55)
#     # print(feature_map.shape)
#
#     plt.figure(figsize=(256, 256))
#     plt.imshow(feature_map.cpu().numpy(), cmap='viridis', interpolation='nearest')  # 使用 viridis 配色方案
#     plt.colorbar()  # 添加颜色条，显示权重值的范围
#     plt.title('Attention Map')  # 图表标题
#
#     plt.imsave('featuremap/LEVIR-256/' + '-' + str(epoch) + ".png", feature_map.cpu().numpy())
#     # save_path = 'featuremap/LEVIR-256/' +  +str(epoch) + ".png"
...
#     #     plt.axis('off')
#     #     # scipy.misc.imsave('feature_map_save//' + str(index) + ".png", feature_map[index - 1])
#     #     plt.imsave('featuremap/LEVIR-256/'+str(index) + "_" +str(epoch) + ".png", feature_map[index - 1].cpu().numpy(),cmap='jet')
