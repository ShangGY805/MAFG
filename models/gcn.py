...
class SpatialGCN(nn.Module):
    ...
    def forward(self, x):
        # b, c, h, w = x.size()   假设中间通道数为3，h*w=64
        node_in1 = self.node_in1(x)
        node_in2 = self.node_in2(x)
        node_in3 = self.node_in3(x)
        b, c, h, w = node_in1.size()
        node_in1 = node_in1.view(b, c, -1).permute(0, 2, 1)  # (b,64,3)
        node_in3 = node_in3.view(b, c, -1)  # (b,3,64)
        node_in2 = node_in2.view(b, c, -1).permute(0, 2, 1)  # (b,64,3)
        AV = torch.bmm(node_in3, node_in2)  # AV=(b,3,3)
        AV = self.softmax(AV)
        AV = torch.bmm(node_in1, AV)  # AV=(b,64,3)
        AV = AV.transpose(1, 2).contiguous()  # AV=(b,3,64)
        AVW = self.conv_wg(AV)
        AVW = self.bn_wg(AVW)
...
class SpectralGCN(nn.Module):
    ...
    def forward(self, x):
        ...
        # Project
        z_idt = torch.matmul(x_sqz, b.transpose(1, 2)) #(C/8,C/4)
