...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout):
        super().__init__()
        self.layers = nn.ModuleList([])  #模块列表，模块打包操作
        #下方Attention对应Transformer中的Multi-Head self_attention模块，而FeedForward对应Transformer中的positional Encoding模块
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                Residual(PreNorm(dim, Attention(dim, heads = heads, dim_head = dim_head, dropout = dropout))),  #Residual是将括号内的操作打包然后返回结果，prenorm是把括号内的操作按dim打包然后返回结果
...
class TransformerDecoder(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout, softmax=True):
        ...
        #下方Residual2和PreNorm2与Transformer中对应的Residual和PreNorm不同之处在于前者的输入为x和x2后者的输出只有x
        #下方Cross_Attention和Attention的区别只在于前者输入有x和m而后者只有x
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
